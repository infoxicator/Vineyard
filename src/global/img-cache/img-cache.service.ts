...
export class ImgCacheService {
  ...
  private normalizeURlWKWview(url: string) {
    let urlIos ='';
    if(url.startsWith(this.file.applicationStorageDirectory)){
     urlIos = normalizeUrlIos(url);
    }else{
     urlIos = `${normalizeUrlIos(this.file.applicationStorageDirectory)}Documents/${normalizeUrlIos(url)}`;
    }
